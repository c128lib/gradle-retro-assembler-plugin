...
This is an open source project hosted on https://github.com/c128lib/gradle-retro-assembler-plugin[GitHub].
In case of any problems when working with the plugin, don't hesitate to create an https://github.com/c128lib/gradle-retro-assembler-plugin/issues[issue].
Feel free to fork and create pull requests.

The roadmap for the project is fairly dynamic, but it can be accessed and reviewed at any moment at https://github.com/orgs/c128lib/projects/3/views/6[this place].
...
* https://github.com/c128lib/ctm-viewer[]
...
----
plugins {
    id "com.github.c128lib.retro-assembler" version "{actualPluginVersion}"
}
...
The plugin is published into Plugins portal: https://plugins.gradle.org/plugin/com.github.c128lib.retro-assembler[https://plugins.gradle.org] and can be used as any other Gradle plugin.
...
dialectVersion:: A version of the selected assembler you want to use. For KickAssembler, see https://github.com/c128lib/asm-ka/releases for list of all available versions.
...
If you want to use `c128lib` set of ASM libraries and also use <<Charpad exports>> and <<Spritepad exports>> configured to use `build` directory as a output, use following snippet:
...
retroProject {
    dialect = "KickAssembler"
    dialectVersion = "{actualKickAssVersion}"
    libDirs = [".ra/deps/c128lib", "build/charpad", "build/spritepad"]

    libFromGitHub("c128lib/common", "develop")
    libFromGitHub("c128lib/chipset", "develop")
    libFromGitHub("c128lib/text", "develop")
    libFromGitHub("c128lib/copper64", "develop")
}
----
...
----
retroProject {
    libFromGitHub "c128lib/common", "1.0.0"
}
----
  ...
  build:
    branches:
      only:
        ...
        - develop
    docker:
      - image: maciejmalecki/c128libci:{actualDockerVersion}
...
One thing you shouldn't modify is `docker/image` - the `maciejmalecki/c128libci:{actualDockerVersion}` is a dedicated image based on Debian Buster that has Java 11 and Vice 3.x preinstalled and is needed to run both KickAssembler and 64spec tests.
